...
| BootScan/RecScan evidence | All three manual relationship modes are active. Distance mode shares sparse requested-pair `FastBootDistIP` rows. UPGMA/NJ modes calculate the full cohort matrix, build one tree per window × `SEQBOOT2` replicate, store relative leaf-path positions, and then interpret every explicit unordered triplet. Source cutoff/overlap runs and triplet-local `MakeScoresBS`/`ProbCalc` evidence independently discover and recalculate events. | Desktop boundary/warning/tree-tie corpus and broad sensitivity comparison |
...
| SiScan evidence | Independent source `GetSSOL` → 15-category/sum → `DoPerms3P`/`MakeZValue2` → `FindMaxZ` → `ShrinkRegionC` discovery runs once per unordered concrete triplet; the baseline/local sister-pair transition resolves recombinant and parents without designated references. The generic oriented prelocator is retired. Fourth-sequence, position/gap, window/step, permutation, multi-run and whole-region controls are operational. Exact telescoping band prefixes and cached deterministic randomization moments are equality-tested against direct enumeration and retain a hard large-tract memory fallback. | Desktop missing/tie/GUI corpus and broad numerical comparison |
| 3Seq evidence | Fused `FindSubSeqTS` all-three-role triplet kernel; source `CheckwrapC` bounded origin extension and linear-complement branch; `CheckSplit3Seq`/`SubPVal` recalibration of continuous post-erasure pieces using full-walk step counts; `Seq3PVals`-equivalent exact and `GetTSPVal` large-walk probabilities; independent discovery/recalculation | Desktop coordinate/tie outputs, precomputed-table rounding and corrected-p corpus |
...
| Event clustering | Source `GetSupers` daughter-distance/overlap score with WPGMA-like average linkage, plus §4.1.4 characterization of every sequence under all three presumed-recombinant orientations. Source-style 60-VNP regions, outside-flank averaging, six-cell category vectors, `CalCR` relabeling/inversion checks, `MakeProperRCorr` SDM filtering, detectable-signal overlap, and the configurable 2-of-3 rule retain co-recombinant descendants even without an independent raw signal. Six seeded JC/NJ bootstrap trees are constructed per bounded cohort, low-support branches are collapsed at the source 50% default, and tree-movement evidence is recorded with cohort/block provenance. Deterministic cohort partitioning retains the detecting triplet in every cohort and places every candidate in one tree bundle, so the runtime cap cannot silently exclude descendants. | Exact `Clearcut` consensus/tree-distance equivalence, full `CheckBSTree` weighting, and desktop event-corpus parity; long regions use tunable balanced site blocks unless the block count covers every site |
| Circular genomes | Opt-in dual-origin scan for methods that need it, native wrapping coordinates, circular-breakpoint deduplication, plots, editing, and masking. Source 3Seq instead runs once through `CheckwrapC`, avoiding an extra origin-dependent test. | Broader circular simulation corpus and RDP5 numerical comparison |
...
3. The enabled RDP method has an independent O(L) source-compatible VNP scan
   ...
   SiScan independently screens the same unordered triplet once with the
   supplied-source fourth-sequence, 15-category, vertical-permutation and
   topology-run workflow; its several significant runs are queued and their
   baseline/local topology transition resolves roles. Fixed-outgroup windows use
...
4. Exact 3SEQ first-passage probabilities are regression-tested against
   exhaustive paths and enter production only through a retained
   `FindSubSeqTS` source-walk ledger; the old statistic-only fallback is gone.
   BootScan's distance, UPGMA and neighbor-joining relationship paths are all
   production options; distance remains the default and fastest path.
