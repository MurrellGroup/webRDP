...
const METHOD_META: Record<MethodName, { family: string; detail: string; limit: string; citation: string }> = {
  ...
  GENECONV: {
    ...
    detail: "RDP5 six-track triplet fragment scan with G-penalized mismatch bridging and source probability calibration.",
    limit: "The active FindSubSeqGCAP6/7 → GetFragsP → GetMaxFragScoreP → CalcKMaxP/GCCalcPValP queue is ported; desktop golden-corpus validation remains in progress.",
    citation: "https://doi.org/10.1006/viro.1999.0058",
  },
  BootScan: {
    family: "Phylogenetic / windowed",
    detail: "RDP5 RecScan batch with shared distance, UPGMA, or neighbor-joining bootstrap relationships and triplet-local source scoring.",
    limit: "The active BSXoverR2 → SEQBOOT2 → FastBootDistIP → distance/UPGMA/NJ tree-position transform → GetPltVal2 → MakeScoresBS/ProbCalc paths are ported; desktop golden-corpus validation remains.",
  ...
  "3Seq": {
    ...
    detail: "RDP5 3Seq: one fused concrete-triplet pass, three queried recombinant roles, triplet-local informative-site compression, and source HGRW probabilities.",
    limit: "FindSubSeqTS/TSXOver and Seq3PVals/GetTSPVal are active. Small walks use exact table-equivalent first-passage probabilities; large walks use RDP5’s SiegmundDiscrete branch. Circular events use the engine’s dual-origin pass; broad desktop golden-corpus validation remains.",
...
function MethodSpecificPlot({ alignment, event, method, window }: { alignment: AlignmentData; event: RdpEvent; method: MethodName; window: number }) {
  ...
  const points = useMemo(() => {
    ...
    const labels: Record<Exclude<MethodName, "3Seq">, string> = {
      ...
      BootScan: "Context only: minor-minus-major window identity (saved evidence uses the source bootstrap topology batch)",
  ...
  return <div className="method-profile">
    ...
    <p>{method === "SiScan" && selectedSignal?.profile?.length ? "The line is the saved source permutation scan, decimated only for project/UI size; zero is the whole-alignment topology, the selected alternative is positive, and the competing alternative is negative. " : ""}{method === "BootScan" ? "This lightweight identity line is positional context, not the BootScan statistic; the source topology support, replicate count and triplet-local probability are reported in the ledger above. " : ""}{selectedSignal ? `${selectedSignal.locator} supplied the shaded method interval; the dashed lines are its breakpoints.` : "No independent locator is stored for this method yet, so the shaded interval is the consensus event."} The saved p-value above comes from the worker statistic and its documented calibration.</p>
  </div>;
}
...
function MethodEvidencePanel({ alignment, event, window, selectedMethod, onSelectMethod, mode = "all" }: {
  ...
  const bootTopologyNames = [
    `${alignment.sequences[event.recombinant]?.name ?? `Sequence ${event.recombinant + 1}`} + ${alignment.sequences[event.majorParent]?.name ?? `Sequence ${event.majorParent + 1}`}`,
    `${alignment.sequences[event.recombinant]?.name ?? `Sequence ${event.recombinant + 1}`} + ${alignment.sequences[event.minorParent]?.name ?? `Sequence ${event.minorParent + 1}`}`,
    `${alignment.sequences[event.majorParent]?.name ?? `Sequence ${event.majorParent + 1}`} + ${alignment.sequences[event.minorParent]?.name ?? `Sequence ${event.minorParent + 1}`}`,
  ];
  return <div className="method-result-explorer">
    ...
    <div className="method-result-detail">
      ...
      {evidence ? <>
        ...
        {selectedSignal?.sourceGeneconv && <div className="source-chi-ledger"><strong>RDP5 GENECONV fragment ledger</strong><span><b>Track {selectedSignal.sourceGeneconv.track + 1}</b> of six source tracks</span><span><b>{selectedSignal.sourceGeneconv.informativeSites.toLocaleString("en-US")}</b> triplet-variable complete sites</span><span><b>{selectedSignal.sourceGeneconv.matchingSites} / {selectedSignal.sourceGeneconv.mismatchSites}</b> matching / mismatch sites</span><span><b>{selectedSignal.sourceGeneconv.mismatchPenalty}</b> mismatch penalty</span><small>Compressed ranks {selectedSignal.sourceGeneconv.startRank + 1}–{selectedSignal.sourceGeneconv.endRank + 1} · raw p {formatP(selectedSignal.sourceGeneconv.rawP)} · {selectedSignal.sourceRoutine}</small></div>}
        {selectedSignal?.sourceBootscan && <div className="source-chi-ledger"><strong>RDP5 BootScan / RecScan topology ledger</strong><span><b>{bootTopologyNames[selectedSignal.sourceBootscan.topology] ?? `Topology ${selectedSignal.sourceBootscan.topology + 1}`}</b> supported tract relationship</span><span><b>{bootTopologyNames[selectedSignal.sourceBootscan.baselineTopology] ?? `Topology ${selectedSignal.sourceBootscan.baselineTopology + 1}`}</b> whole-alignment baseline</span><span><b>{(selectedSignal.sourceBootscan.bootstrapSupport * 100).toFixed(1)}% of {selectedSignal.sourceBootscan.bootstrapReplicates}</b> maximum bootstrap support</span><span><b>{selectedSignal.sourceBootscan.runWindows} windows</b> source cutoff/overlap run</span><span><b>{selectedSignal.sourceBootscan.tractPairMatches} / {selectedSignal.sourceBootscan.tractInformativeSites}</b> tract pair matches / variable complete sites</span><span><b>{selectedSignal.sourceBootscan.tractPairMatches + selectedSignal.sourceBootscan.backgroundPairMatches} / {selectedSignal.sourceBootscan.informativeSites}</b> whole-triplet pair matches / variable complete sites</span><small>{selectedSignal.sourceBootscan.window}/{selectedSignal.sourceBootscan.step} nt window/step · raw p {formatP(selectedSignal.sourceBootscan.rawP)} · {selectedSignal.sourceBootscan.relationshipMode} mode · {selectedSignal.sourceRoutine}</small></div>}
        {selectedSignal?.sourceThreeSeq && <div className="source-chi-ledger"><strong>RDP5 3Seq walk ledger</strong><span><b>{selectedSignal.sourceThreeSeq.informativeSites.toLocaleString("en-US")}</b> full target-matches-one-parent sites</span><span><b>{selectedSignal.sourceThreeSeq.upSteps} / {selectedSignal.sourceThreeSeq.downSteps}</b> +1 / −1 steps after source direction selection</span><span><b>{selectedSignal.sourceThreeSeq.descent}</b> {selectedSignal.sourceThreeSeq.splitRefined ? `continuous-piece excursion (${selectedSignal.sourceThreeSeq.fullDescent} before split)` : "maximum HGRW excursion"}</span>{selectedSignal.sourceThreeSeq.splitRefined && <span><b>{selectedSignal.sourceThreeSeq.splitInformativeSites}</b> informative sites in the retained CheckSplit3Seq piece</span>}<span><b>{selectedSignal.sourceThreeSeq.sourceWrap ? selectedSignal.sourceThreeSeq.linearComplement ? "source wrap → linear complement" : "origin-spanning source wrap" : "non-wrapping source excursion"}</b> CheckwrapC result</span><span><b>{selectedSignal.sourceThreeSeq.probabilityMode.replaceAll("-", " ")}</b> probability path</span><small>Cycle {selectedSignal.sourceThreeSeq.cycle + 1} · {selectedSignal.sourceThreeSeq.direction > 0 ? "forward descent" : "reverse ascent"} · raw p {formatP(selectedSignal.sourceThreeSeq.rawP)} · {selectedSignal.sourceRoutine}</small></div>}
  ...
  return (
    <aside className="inspector">
      ...
      <div className="inspector-section">
        ...
        {(event.methodSignals?.filter((signal) => signal.method !== "shared-screen").length ?? 0) > 0 && <details className="method-signal-list" open><summary>Method-specific interval calls</summary><div>{event.methodSignals?.filter((signal) => signal.method !== "shared-screen").map((signal, index) => <button type="button" key={`${signal.method}-${signal.start}-${signal.end}-${index}`} onClick={() => onOpenMethod(signal.method as MethodName)}><b>{signal.method}</b><span>{signal.start + 1}–{signal.end}{signal.wraps ? " · wraps origin" : ""}</span><small>{signal.locator} · {signal.statistic.toPrecision(4)}{signal.sourceGeneconv ? ` · track ${signal.sourceGeneconv.track + 1} · ${signal.sourceGeneconv.informativeSites} compressed sites · penalty ${signal.sourceGeneconv.mismatchPenalty} · p ${formatP(signal.sourceGeneconv.rawP)}` : ""}{signal.sourceBootscan ? ` · ${signal.sourceBootscan.relationshipMode} · topology ${signal.sourceBootscan.topology + 1} vs baseline ${signal.sourceBootscan.baselineTopology + 1} · ${(signal.sourceBootscan.bootstrapSupport * 100).toFixed(1)}%/${signal.sourceBootscan.bootstrapReplicates} · ${signal.sourceBootscan.runWindows} windows · p ${formatP(signal.sourceBootscan.rawP)}` : ""}{signal.sourceThreeSeq ? ` · ${signal.sourceThreeSeq.informativeSites} compressed sites · ${signal.sourceThreeSeq.upSteps}/${signal.sourceThreeSeq.downSteps} steps · excursion ${signal.sourceThreeSeq.descent}${signal.sourceThreeSeq.splitRefined ? `/${signal.sourceThreeSeq.fullDescent} split/full` : ""} · ${signal.sourceThreeSeq.sourceWrap ? signal.sourceThreeSeq.linearComplement ? "wrap-complement" : "wrap" : "linear"} · p ${formatP(signal.sourceThreeSeq.rawP)}` : ""}{signal.sourceChi ? ` · ${signal.sourceChi.informativeSites} compressed sites · H=${signal.sourceChi.halfWindow} · grown χ² ${signal.sourceChi.boundaryStatistics.map((value) => value.toPrecision(3)).join("/")}` : ""}{signal.method === "SiScan" ? ` · ${signal.outgroup === null ? "randomized fourth sequence" : signal.outgroup === undefined ? signal.outgroupMode ?? "source outgroup" : `outgroup ${alignment.sequences[signal.outgroup]?.name ?? signal.outgroup + 1}`} · ${signal.permutations ?? "?"} permutations${signal.sourceSiScan ? ` · ${signal.sourceSiScan.runWindows} windows · p ${formatP(signal.sourceSiScan.rawP)} · triplet ${signal.sourceSiScan.topologyTriplet.map((member) => alignment.sequences[member]?.name ?? member + 1).join(" / ")}` : ""}` : ""}</small></button>)}</div></details>}
        ...
        {activeCoRecombinantSet && <details className="ancestral-cluster-card co-recombinant-screen" open><summary><span>RDP5 co-recombinant screen</span><b>{activeCoRecombinantSet.sequenceMembers.length} sequence{activeCoRecombinantSet.sequenceMembers.length === 1 ? "" : "s"}</b></summary><div>
          ...
          {activeTreeBootstrap && <small>{activeTreeBootstrap.replicates} seeded NJ bootstrap replicates · branches below {Math.round(activeTreeBootstrap.cutoff * 100)}% collapsed · {activeTreeBootstrap.cohortTaxa} taxa per tree cohort across {activeTreeBootstrap.cohortCount ?? 1} cohort{(activeTreeBootstrap.cohortCount ?? 1) === 1 ? "" : "s"}{activeTreeBootstrap.candidateComplete ? ` · all ${activeTreeBootstrap.sourceSequenceCount} candidates covered` : ` · ${activeTreeBootstrap.sourceSequenceCount} source taxa`}{activeTreeBootstrap.exactSiteBootstrap ? " · exact site bootstrap" : " · balanced site-block bootstrap"}</small>}
...
export default function Home() {
    ...
    worker.onmessage = (message) => {
      ...
      if (payload.type === "progress") {
        ...
        setMetrics({
          ...
          geneconvSignalTruncations: payload.geneconvSignalTruncations,
          chiSignalTruncations: payload.chiSignalTruncations,
          bootscanSignalTruncations: payload.bootscanSignalTruncations,
          bootscanBatch: payload.bootscanBatch,
        ...
        appendAudit(config.auditAction ?? (retainedEvents.length ? "Completed unresolved-sequence rescan" : "Completed full scan"), `${config.auditContext ? `${config.auditContext} ` : ""}${payload.comparisons.toLocaleString("en-US")} triplets tested; ${resultEvents.length} new consensus hypotheses retained${payload.events.length !== resultEvents.length ? ` after suppressing ${payload.events.length - resultEvents.length} duplicate of an accepted event` : ""}${retainedEvents.length ? ` alongside ${retainedEvents.length} preserved hypotheses` : ""}${payload.detectionCycle?.enabled ? `; RDP5 cyclical detection selected ${payload.detectionCycle.eventsApplied} events across ${payload.detectionCycle.passes} passes and redid ${payload.detectionCycle.redoComparisons.toLocaleString("en-US")} affected triplets` : ""}${payload.disassembly?.components ? `; ${payload.disassembly.components} tract-only analysis components generated from ${payload.disassembly.appliedEvents} accepted events` : ""}${payload.disassembly?.unresolvedLineageEvents ? `; ${payload.disassembly.unresolvedLineageEvents} nested event lineage${payload.disassembly.unresolvedLineageEvents === 1 ? " was" : "s were"} withheld because its accepted predecessor component could not be rebuilt` : ""}${payload.rdpSignalTruncations ? `; ${payload.rdpSignalTruncations} lower-ranked RDP excursions exceeded the per-triplet retention cap` : ""}${payload.geneconvSignalTruncations ? `; ${payload.geneconvSignalTruncations} lower-ranked GENECONV fragments exceeded their cap` : ""}${payload.chiSignalTruncations ? `; ${payload.chiSignalTruncations} lower-ranked MAXCHI/CHIMAERA peak pairs exceeded their cap` : ""}${payload.bootscanSignalTruncations ? `; ${payload.bootscanSignalTruncations} raw BootScan topology runs exceeded the batch-copy cap` : ""}${payload.bootscanBatch ? `; BootScan shared ${payload.bootscanBatch.usedPairs.toLocaleString("en-US")} pair rows across ${payload.bootscanBatch.triplets.toLocaleString("en-US")} concrete triplets` : ""}.`);
        worker.terminate();
  ...
  const toggleMethod = (method: MethodName) => {
    setOptions((current) => {
      const alreadyEnabled = current.methods.includes(method);
      const methods = alreadyEnabled
      ...
      return {
        ...
        bootstrapReplicates: method === "BootScan" && !alreadyEnabled && current.bootstrapReplicates < 2
          ? DEFAULT_OPTIONS.bootstrapReplicates
          : current.bootstrapReplicates,
      };
    });
  };
  ...
  const exportResults = (kind: "json" | "csv") => {
    ...
    events.forEach((event, index) => {
      ...
      event.methodSignals?.filter((signal) => signal.method !== "shared-screen").map((signal) => `${signal.method}:${signal.start + 1}-${signal.end}${signal.wraps ? "(wrap)" : ""}:${signal.locator}${signal.sourceGeneconv ? `:track=${signal.sourceGeneconv.track + 1}:compressed=${signal.sourceGeneconv.informativeSites}:score=${signal.sourceGeneconv.fragmentScore}:penalty=${signal.sourceGeneconv.mismatchPenalty}:rawp=${signal.sourceGeneconv.rawP}` : ""}${signal.sourceBootscan ? `:relationship=${signal.sourceBootscan.relationshipMode}:topology=${signal.sourceBootscan.topology + 1}:baseline=${signal.sourceBootscan.baselineTopology + 1}:support=${signal.sourceBootscan.bootstrapSupport}:replicates=${signal.sourceBootscan.bootstrapReplicates}:windows=${signal.sourceBootscan.runWindows}:tractmatches=${signal.sourceBootscan.tractPairMatches}/${signal.sourceBootscan.tractInformativeSites}:rawp=${signal.sourceBootscan.rawP}:windowstep=${signal.sourceBootscan.window}/${signal.sourceBootscan.step}` : ""}${signal.sourceThreeSeq ? `:compressed=${signal.sourceThreeSeq.informativeSites}:steps=${signal.sourceThreeSeq.upSteps}/${signal.sourceThreeSeq.downSteps}:excursion=${signal.sourceThreeSeq.descent}:full-excursion=${signal.sourceThreeSeq.fullDescent ?? signal.sourceThreeSeq.descent}:split-sites=${signal.sourceThreeSeq.splitInformativeSites ?? ""}:checksplit=${signal.sourceThreeSeq.splitRefined === true}:source-wrap=${signal.sourceThreeSeq.sourceWrap === true}:linear-complement=${signal.sourceThreeSeq.linearComplement === true}:mode=${signal.sourceThreeSeq.probabilityMode}:rawp=${signal.sourceThreeSeq.rawP}` : ""}${signal.sourceChi ? `:track=${signal.sourceChi.track + 1}:compressed=${signal.sourceChi.informativeSites}:halfwindow=${signal.sourceChi.halfWindow}:boundarychi=${signal.sourceChi.boundaryStatistics.join("/")}:growth=${signal.sourceChi.growthWidths.join("/")}` : ""}${signal.method === "SiScan" ? `:outgroup=${signal.outgroup === null ? "randomized" : signal.outgroup === undefined ? signal.outgroupMode ?? "unspecified" : alignment.sequences[signal.outgroup]?.name ?? signal.outgroup + 1}:permutations=${signal.scanPermutations ?? "?"}/${signal.permutations ?? "?"}:score=${signal.scoreFamily ?? "?"}-${signal.pattern ?? "?"}:topology=${signal.baselineTopology ?? "?"}->${signal.inferredTopology ?? "?"}${signal.sourceSiScan ? `:rawp=${signal.sourceSiScan.rawP}:windows=${signal.sourceSiScan.runWindows}:windowstep=${signal.sourceSiScan.window}/${signal.sourceSiScan.step}:triplet=${signal.sourceSiScan.topologyTriplet.join("/")}` : ""}` : ""}`).join(";") ?? "",
          ...
          <div className="sidebar-scroll">
            ...
            <section className="settings-section controls">
              ...
              <details><summary>Advanced controls</summary><div className="advanced-controls">
                ...
                {options.methods.includes("GENECONV") && <><label><span>GENECONV G-scale <b>{Math.round(options.geneconvGScale)}</b></span><input type="range" min={0} max={10} step={1} value={Math.round(options.geneconvGScale)} onChange={(event) => setOption("geneconvGScale", Number(event.target.value))}/><small>Original integer source parameter; 0 forbids mismatch bridging and 1 is the RDP5 default.</small></label><label><span>GENECONV fragments retained/triplet <b>{options.geneconvSignalsPerTriplet}</b></span><input type="range" min={8} max={256} step={8} value={options.geneconvSignalsPerTriplet} onChange={(event) => setOption("geneconvSignalsPerTriplet", Number(event.target.value))}/><small>The source overlap queue is fully drained; this only caps rows copied back from WASM.</small></label></>}
                {options.methods.includes("BootScan") && <details className="parity-controls"><summary>BootScan / RecScan source controls · {options.bootscanRelationshipMode}</summary><div>
                  <label><span>Window <b>{options.bootscanWindow} nt</b></span><input type="range" min={20} max={Math.max(20, Math.min(2000, Math.floor(alignment.length / 2)))} step={10} value={Math.min(options.bootscanWindow, Math.max(20, Math.floor(alignment.length / 2)))} onChange={(event) => setOption("bootscanWindow", Number(event.target.value))}/><small>Alignment positions, including invariant sites; RDP5 default 200.</small></label>
                  <label><span>Step <b>{options.bootscanStep} nt</b></span><input type="range" min={1} max={Math.max(1, Math.min(500, Math.floor(alignment.length / 4)))} step={1} value={Math.min(options.bootscanStep, Math.max(1, Math.floor(alignment.length / 4)))} onChange={(event) => setOption("bootscanStep", Number(event.target.value))}/><small>RDP5 default 20; smaller values increase breakpoint resolution and runtime.</small></label>
                  <label><span>Topology support cutoff <b>{Math.round(options.bootscanCutoff * 100)}%</b></span><input type="range" min={0.5} max={0.99} step={0.01} value={options.bootscanCutoff} onChange={(event) => setOption("bootscanCutoff", Number(event.target.value))}/></label>
                  <label><span>Signals copied from batch <b>{options.bootscanSignals.toLocaleString("en-US")}</b></span><input type="range" min={512} max={50000} step={512} value={Math.min(50000, options.bootscanSignals)} onChange={(event) => setOption("bootscanSignals", Number(event.target.value))}/><small>Raw topology runs are scored after the shared batch; any overflow is recorded in project provenance.</small></label>
                  <label className="select-label"><span>Relationship mode</span><select value={options.bootscanRelationshipMode} onChange={(event) => setOption("bootscanRelationshipMode", event.target.value as AnalysisOptions["bootscanRelationshipMode"])}><option value="distance">Pairwise distances · fastest</option><option value="neighbor-joining">Neighbor-joining trees · recommended tree mode</option><option value="upgma">UPGMA trees</option></select><small>{options.bootscanRelationshipMode === "distance" ? "Only pairs requested by concrete triplets are calculated; one shared matrix is reused by all triplets." : "A full active-cohort tree is built once per window × bootstrap replicate, then its relative tree positions are reused by every concrete triplet. Runtime rises roughly cubically with sequence count."}</small></label>
                  {options.bootscanRelationshipMode !== "distance" && <div className="source-fixed-setting"><span>Projected shared-tree workload</span><b>{((Math.floor(alignment.length / Math.max(1, options.bootscanStep)) + 2) * Math.max(2, options.bootstrapReplicates) * (options.circular ? 2 : 1)).toLocaleString("en-US")} trees</b><small>Up to {chooseThree(alignment.sequences.length).toLocaleString("en-US")} requested concrete triplets reuse each tree; {(alignment.sequences.length * (alignment.sequences.length - 1) / 2).toLocaleString("en-US")} active-cohort pairs feed every bootstrap matrix before disassembly creates any extra components.</small></div>}
                </div></details>}
                {options.methods.includes("3Seq") && <details className="parity-controls"><summary>3Seq source controls</summary><div>
                  <label><span>Exact probability work cap <b>{options.threeSeqExactOperations.toLocaleString("en-US")}</b></span><input type="range" min={10000} max={5000000} step={10000} value={Math.min(5000000, options.threeSeqExactOperations)} onChange={(event) => setOption("threeSeqExactOperations", Number(event.target.value))}/><small>On-demand Seq3PVals-equivalent DP operations. Larger walks switch to RDP5 GetTSPVal’s SiegmundDiscrete path; this changes speed and the exact/approximation boundary, not triplet coverage.</small></label>
                  <div className="source-fixed-setting"><span>Triplet traversal</span><b>One fused pass · 3 targets</b><small>Invariant, incomplete, triallelic, and neither-parent-match sites are discarded separately for each target walk.</small></div>
                </div></details>}
                {options.methods.includes("SiScan") && <details className="parity-controls"><summary>SiScan source controls</summary><div>
                  ...
                  <label><span>Window <b>{options.siskanWindow} nt</b></span><input type="range" min={12} max={Math.max(12, Math.min(2000, alignment.length))} step={1} value={Math.min(options.siskanWindow, Math.max(12, Math.min(2000, alignment.length)))} onChange={(event) => setOption("siskanWindow", Number(event.target.value))}/><small>Source default 200 alignment positions.</small></label>
                  <label><span>Step <b>{options.siskanStep} nt</b></span><input type="range" min={1} max={Math.max(1, Math.min(500, alignment.length))} step={1} value={Math.min(options.siskanStep, Math.max(1, Math.min(500, alignment.length)))} onChange={(event) => setOption("siskanStep", Number(event.target.value))}/><small>Source default 20. Smaller steps increase plot and breakpoint resolution at proportional runtime cost.</small></label>
                  ...
                  <p className="setting-help">Each unordered concrete triplet is screened once by the source 15-category workflow. A change from the whole-alignment sister pair to the run’s sister pair resolves the recombinant and its two parents without a designated reference sequence.</p>
                </div></details>}
                <label><span>Bootstrap replicates <b>{options.bootstrapReplicates}</b></span><input type="range" min={options.methods.includes("BootScan") ? 2 : 0} max={500} step={options.methods.includes("BootScan") ? 1 : 25} value={options.bootstrapReplicates} onChange={(event) => setOption("bootstrapReplicates", Number(event.target.value))}/>{options.methods.includes("BootScan") && <small>Shared by BootScan; at least two replicates are required in every relationship mode.</small>}</label>
                ...
                <details className="parity-controls"><summary>BURT + event-clustering controls</summary><div>
                  ...
                  {options.ancestralClustering && <>
                    ...
                    <label><span>Tree cohort cap <b>{options.clusterTreeTaxaLimit} taxa</b><small>Every candidate is assigned to a bounded cohort with the detecting triplet; lowering this cap increases cohort count without dropping sequences.</small></span><input type="range" min={4} max={100} step={4} value={options.clusterTreeTaxaLimit} onChange={(event) => setOption("clusterTreeTaxaLimit", Number(event.target.value))}/></label>
        ...
        <section className="workspace">
          ...
          <div className="workspace-content">
            ...
            {tab === "methods" && <>
              ...
              <div className="fidelity-banner"><strong>Validation state · scientific alpha</strong><p>The active engine now includes multi-signal RDP5 VNP scanning; the full six-track GENECONV fragment queue; the author-source distance, UPGMA, and neighbor-joining BootScan/RecScan batches; source MAXCHI/CHIMAERA compressed-site profiles; independent once-per-triplet SiScan with source outgroup/permutation calibration; the fused all-three-role 3Seq FindSubSeqTS/Seq3PVals path; source BURT breakpoint refinement; recursive erase/extract signal disassembly; source-style ancestral event merging; all-sequence co-recombinant screening; seeded JC/NJ bootstrap tree evidence; and the default RDP5 recombinant-identification consensus. Remaining numerical-parity gates include desktop BootScan tree tie/rounding goldens, optional logistic/neural role selectors, exact Clearcut/CheckBSTree behavior, and a broad RDP5 desktop golden regression corpus.</p><span>Detector specification: author-supplied RDP5 VB/native source only.</span></div>
