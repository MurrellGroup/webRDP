...
| --- | --- | --- | --- |
| `FindSubSeqPB3` / `FindSubSeqPB6` | `scan_rdp5_triplet` category compression in `assembly/index.ts` | Ported | Exact two-equal/one-different VNP categories and all-three-category gate; missing-data/tie corpus remains |
...
| `BenHMM` + `DoHMMCyclesSerial` | Source mode in `public/rdp-burt.js` | Ported | Fixed 3 states, 21 starts, 100-iteration ceiling, pseudocounts, transition initialization, source rejection-sampled emission anchors, MSVC 15-bit RNG, predecessor-offset backtrace and sentinel behavior |
| `MatchBPtoCI` / `PolishBP` | BURT switch selection and posterior confidence bounds | Substantially ported | 95/99% intervals and candidate matching operational; exact informative-index tie/reversion corpus remains |
...
The worker no longer uses a common non-RDP discovery seed. Directional method
